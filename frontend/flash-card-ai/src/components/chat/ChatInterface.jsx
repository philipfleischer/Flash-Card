...
//const ChatInterface = ({ documentStatus }) => {
const ChatInterface = ({ documentStatus, documentError }) => {
  ...
  if (documentStatus && documentStatus !== 'ready') {
    return (
      <div className="p-6 bg-white border border-slate-200/60 rounded-2xl">
        <p className="text-sm text-slate-600">
          Document is still processing. Status:{' '}
          <span className="font-semibold">{documentStatus}</span>
        </p>
        {documentStatus === 'failed' && documentError && (
          <p className="text-xs text-red-600 mt-2">
            Error: <span className="font-mono">{documentError}</span>
          </p>
        )}
        <p className="text-xs text-slate-500 mt-2">
          Wait a moment and refresh. Chat becomes available when status is <b>ready</b>.
        </p>
      </div>
    );
  }
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const assistantMessage = {
        role: 'assistant',
        content: response.data?.answer ?? response.data?.data?.answer ?? 'No answer returned.',
        timestamp: new Date(),
        relevantChunks: response.data?.relevantChunks ?? response.data?.data?.relevantChunks ?? [],
      };
      ...
      <div className="flex-1 p-6 overflow-y-auto bg-linear-to-br from-slate-50/50 via-white/50 to-slate-50/50">
        {history.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-sm text-slate-500">Ask me anything abot the document!</p>
          </div>
        ) : (
